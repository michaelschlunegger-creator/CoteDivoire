...
import { activityReservations,feedback,registrations } from "@/db/schema";
...
export default async function Dashboard(){
  ...
  try{
    const[registrationRows,feedbackRows,reservationRows]=await Promise.all([
  ...
  return <PageShell><main><section className="page-hero participant-hub-hero"><div className="page-hero-inner"><p className="eyebrow">PARTICIPANT HUB</p><h1>Welcome, {welcomeName}.</h1><p>Your personalized place for today’s agenda, experience reservations, registration status, private feedback and official material.</p></div></section><div className="dashboard-shell"><div className="dashboard-head"><div><span className="status-pill">Securely signed in</span><h1>Your event hub</h1><p>{user.email}</p></div><Link className="button button-secondary" href="/api/auth/signout">Sign out</Link></div><ParticipantPlanner initialActivityIds={activityIds} activities={activities}/><section className="participant-survey-cta"><div><span className="tag">CONFIDENTIAL · 20 QUESTIONS</span><h2>Help set the next benchmark.</h2><p>Evaluate the technical programme, speakers, networking, venue, hospitality, travel support and digital experience in one professionally structured survey.</p><div className="participant-survey-facts"><span><b>20</b> focused questions</span><span><b>≈ 6</b> minutes</span><span><b>100%</b> private</span></div></div><Link className="button" href="/feedback">Start participant survey →</Link></section><div className="dashboard-grid participant-account-grid"><section className="dashboard-card"><h2>Event registration</h2>{registration?<><span className="status-pill">{registration.status}</span><div className="profile-row"><span>Name</span><strong>{welcomeName}</strong></div><div className="profile-row"><span>Organization</span><strong>{registration.organization}</strong></div><div className="profile-row"><span>Participation</span><strong>{registration.participationType}</strong></div><div className="profile-row"><span>Submitted</span><strong>{registration.createdAt.slice(0,10)}</strong></div></>:<><p className="section-lead" style={{fontSize:15}}>Complete your event registration to add your first and last name and unlock the full participant experience.</p><Link className="button" href="/register">Complete registration →</Link></>}</section><aside className="dashboard-card"><h2>Quick access</h2><div className="admin-list"><Link className="admin-list-item" href="/resources"><span>Official downloads</span><strong>→</strong></Link><Link className="admin-list-item" href="/highlights"><span>Daily highlights</span><strong>→</strong></Link><Link className="admin-list-item" href="/feedback"><span>Private participant survey</span><strong>→</strong></Link></div><div className="trust-note"><span>✦</span><span>Your account, reservations and feedback are private. Only you and authorized administrators can view them.</span></div></aside></div><section className="dashboard-card participant-feedback-history"><div className="dashboard-head" style={{marginBottom:12}}><div><span className="tag">PRIVATE TO YOU</span><h2>Your survey history</h2></div><Link className="button button-small" href="/feedback">New response</Link></div>{responses.length?<div className="response-list">{responses.map(response=><article key={response.id}><div><span className="tag">{response.eventDay}</span><strong>{response.category}</strong><p>{response.message}</p></div><div><b>{response.rating}/5</b><small>{response.createdAt.slice(0,10)}</small></div></article>)}</div>:<p className="section-lead" style={{fontSize:15}}>You have not submitted a participant survey yet.</p>}</section></div></main></PageShell>;
}
